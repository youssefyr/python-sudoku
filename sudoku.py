...
def highlightbox():
    for k in range(2):
        pygame.draw.line(Window, BLACK, (x * diff-3, (z + k)*diff), (x * diff + diff + 3, (z + k)*diff), 7)
        pygame.draw.line(Window, BLACK, ( (x + k)* diff, z * diff ), ((x + k) * diff, z * diff + diff), 7) 
    ...
    for i in range (9):
        for j in range (9):
            if defaultgrid[i][j]!= 0:
                ...
                Window.blit(text1, (i * diff + (diff - text_width) / 2, j * diff + (diff - text_height) / 2))   
    for l in range(10):
        if l % 3 == 0 :
            thick = 3
        else:
            thick = 1
        pygame.draw.line(Window, BLACK, (0, l * diff), (500, l * diff), thick)
        pygame.draw.line(Window, BLACK, (l * diff, 0), (l * diff, 500), thick)
...
def startgame():
    global flag,flag1,flag2,rs,value,defaultgrid,x,y,z,error
    flag=True  
    flag1 = 0
    flag2 = 0
    rs = 0
